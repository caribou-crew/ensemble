...
// e.g. "$HOME/dev/exp/local-stack") was copied through literally
// instead of being resolved.
func TestExpandEnvVarsDefaultResolvesNestedBareVar(t *testing.T) {
	got, err := expandEnvVars(
		[]byte("dir: ${LOCAL_STACK_DIR:-$HOME/dev/exp/local-stack}"),
		lookupFrom(map[string]string{"HOME": "/Users/steven"}),
	)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if string(got) != "dir: /Users/steven/dev/exp/local-stack" {
		t.Fatalf("got %q", got)
	}
}

func TestExpandEnvVarsDefaultNestedVarMissingErrors(t *testing.T) {
	_, err := expandEnvVars([]byte("dir: ${LOCAL_STACK_DIR:-$HOME/x}"), lookupFrom(nil))
	if err == nil {
		t.Fatal("expected an error, got nil")
	}
...
services:
  bff:
    run: "node dist/main.js"
    port: 8003
    dir: ${LOCAL_STACK_DIR:-$HOME/dev/exp/local-stack}
`)
	...
	if want := "/Users/steven/dev/exp/local-stack"; c.Services["bff"].Dir != want {
