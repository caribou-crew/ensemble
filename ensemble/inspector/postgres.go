...
func (p *PostgresDriver) Rows(ctx context.Context, table string, limit, offset int) ([]map[string]any, error) {
	orderBy, err := p.orderColumn(ctx, table)
	if err != nil {
		return nil, err
	}
	query := fmt.Sprintf("SELECT * FROM %s", quoteIdentPG(table))
	if orderBy != "" {
		query += " ORDER BY " + quoteIdentPG(orderBy)
	}
...
func (p *PostgresDriver) orderColumn(ctx context.Context, table string) (string, error) {
	pk, hasPK, err := p.primaryKeyColumn(ctx, table)
	if err != nil {
		return "", err
	}
	if hasPK {
