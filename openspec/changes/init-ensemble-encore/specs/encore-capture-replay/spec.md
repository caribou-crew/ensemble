# encore-capture-replay

## ADDED Requirements

### Requirement: Runner-agnostic capture
`encore run --flow <name> -- <command>` SHALL execute the user's test command
...
- **WHEN** encore runs a Playwright flow while ensemble is up
...
- **WHEN** encore runs without ensemble
...
`encore replay --ref <flow>` SHALL serve a blessed recording as mocks from the
