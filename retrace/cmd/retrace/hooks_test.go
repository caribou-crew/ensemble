...
// The per-flow half runs the flow's own preflight before its setup: a seed
// step should not run against a flow whose own precondition just failed.
func TestRunFlowPreconditionsOrdersPreflightThenSetup(t *testing.T) {
	dir := t.TempDir()
	log := filepath.Join(dir, "order.log")
	var stderr bytes.Buffer
	...
	if err := runFlowPreconditions(context.Background(), fl, "app", "flow", dir, &stderr); err != nil {
		t.Fatalf("runFlowPreconditions: %v", err)
	}

	got := strings.Join(readLog(t, log), ",")
	if want := "flow-preflight,setup"; got != want {
		t.Errorf("order = %q, want %q", got, want)
	}
}

func TestRunFlowPreconditionsFailingPreflightSkipsSetup(t *testing.T) {
	dir := t.TempDir()
	log := filepath.Join(dir, "order.log")
	var stderr bytes.Buffer

	fl := config.Flow{
		Preflight: []string{"exit 1"},
		Setup:     []string{touchCmd(log, "setup")},
	}
	if err := runFlowPreconditions(context.Background(), fl, "app", "flow", dir, &stderr); err == nil {
		t.Fatal("a failing flow preflight must stop before setup")
	}
	if got := readLog(t, log); len(got) != 0 {
		t.Errorf("setup ran after the flow preflight failed: %v", got)
	}
}

// The global preflight's scope is the stack, not any one flow. Stamping it
// with a flow name would be a lie a shared seed script could act on — so
// RETRACE_FLOW is empty here, while RETRACE_APP still identifies the app.
func TestRunGlobalPreflightDoesNotClaimAFlow(t *testing.T) {
	dir := t.TempDir()
	out := filepath.Join(dir, "env.txt")
	var stderr bytes.Buffer

	cfg := &config.Config{Preflight: []string{
		"printf '%s|%s' \"$RETRACE_APP\" \"$RETRACE_FLOW\" > " + out,
	}}
	if err := runGlobalPreflight(context.Background(), cfg, "storefront", dir, &stderr); err != nil {
		t.Fatalf("runGlobalPreflight: %v", err)
	}
	b, err := os.ReadFile(out)
	if err != nil {
		t.Fatalf("read: %v", err)
	}
	if string(b) != "storefront|" {
		t.Errorf("global preflight env = %q, want %q (no flow claimed)", b, "storefront|")
	}
}

// A flow with no entry in retrace.yaml has no hooks and must capture exactly
// as it did before this feature existed.
func TestPreconditionsWithNoHooksAreANoOp(t *testing.T) {
	var stderr bytes.Buffer
	dir := t.TempDir()
	if err := runGlobalPreflight(context.Background(), &config.Config{}, "app", dir, &stderr); err != nil {
		t.Fatalf("no global preflight must run clean, got: %v", err)
	}
	if err := runFlowPreconditions(context.Background(), config.Flow{}, "app", "flow", dir, &stderr); err != nil {
