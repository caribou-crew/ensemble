...
Recording more than one flow:
  ...
  name order, in one process — the form to reach for when driving retrace from
  an agent, since a flow added to the config is picked up without changing the
  command. Each flow runs its own flows.<name>.command. A command given after
  the "--" separator overrides that and is valid only for a single flow.
  --json emits one object for --flow, and an array for --flows or bare run:
  the document's shape follows the invocation, never the number of flows that
