...
func TestTheLastMatchingBodyGlobWins(t *testing.T) {
	...
	// flowlens's own test (test/wire-rules.test.mjs:86) asserts this third
...
// covers two flowlens cases (test/wire-rules.test.mjs:56,60) the brief's
// table omitted.
...
// flowlens's matchesFieldGlob, which splits on '.' without filtering out
...
// which does filter (matching flowlens's own path splitter).
