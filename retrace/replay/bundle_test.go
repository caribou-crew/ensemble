...
func TestLoadBundleRefusesACorruptWireLine(t *testing.T) {
	...
	if _, err := LoadBundle(dir, "", nil); err != nil {
...
func TestLoadBundleRefusesAnEmptyDirectoryArgument(t *testing.T) {
	...
	if _, err := LoadBundle("", "", nil); err == nil {
		t.Fatal(`LoadBundle("", "", nil) was accepted`)
	}
}
...
func TestLoadBundleRefusesAnExchangeRecordedWithContentEncoding(t *testing.T) {
	...
	if _, err := LoadBundle(writeBundle(t, runs.Counts{Calls: 1, Recorded: true}, []trace.Hop{clean}), "", nil); err != nil {
	...
	if _, err := LoadBundle(writeBundle(t, runs.Counts{Calls: 1, Recorded: true}, []trace.Hop{identity}), "", nil); err != nil {
...
func TestLoadBundleRefusesATruncatedRecordedRequestBody(t *testing.T) {
	...
	if _, err := LoadBundle(writeBundle(t, runs.Counts{Calls: 1, Recorded: true}, []trace.Hop{whole}), "", nil); err != nil {
...
func TestLoadBundleRefusesATruncatedRecordedResponseBody(t *testing.T) {
	...
	b, err := LoadBundle(writeBundle(t, runs.Counts{Calls: 1, Recorded: true}, []trace.Hop{whole}), "", nil)
	if err != nil {
...
func TestLoadBundleRefusesARecordedPartialResponse(t *testing.T) {
	...
	for _, c := range []struct {
		...
		t.Run(c.name, func(t *testing.T) {
			...
			_, err := LoadBundle(writeBundle(t, runs.Counts{Calls: 1, Recorded: true}, []trace.Hop{h}), "", nil)
			if err == nil {
	...
	t.Run("Accept-Ranges alone still loads and serves", func(t *testing.T) {
		...
		b, err := LoadBundle(writeBundle(t, runs.Counts{Calls: 1, Recorded: true}, []trace.Hop{h}), "", nil)
		if err != nil {
...
func TestLoadBundleRefusesARecordedRequestContentEncoding(t *testing.T) {
	...
	b, err := LoadBundle(writeBundle(t, runs.Counts{Calls: 1, Recorded: true}, []trace.Hop{plain}), "", nil)
	if err != nil {
