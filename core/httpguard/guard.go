// Package httpguard is the browser-facing protection every unauthenticated
// loopback control plane in this repo sits behind: ensemble's dashboard API
// and retrace's marker door, replay server and review server. It lives in
// core so there is exactly ONE copy of these checks — a second, inlined
// copy of "just the Sec-Fetch-Site part" is strictly weaker than this and
// is how a listener ends up open to DNS rebinding.
package httpguard
...
//
// Handler closes both: the Host header must name a host we're actually
...
// host/origin matching (Sec-Fetch-Site is still enforced). Pass nil for a
// listener that should answer only as loopback — nil is the SAFE zero
// value here, never "no allow-list, so allow anything".
//
// It runs ahead of every route, including GETs — reading captured traffic
// is exactly what a rebinding attack is after.
func Handler(allowedHosts []string, h http.Handler) http.Handler {
	allowed := newHostSet(allowedHosts)
...
// writeErr mirrors ensemble/server's error shape ({"error": msg}) so a
// rejection looks identical to every existing client after the extraction.
// The guard can no longer borrow the server package's writer, so it owns
// this one.
