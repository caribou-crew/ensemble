...
func TestTwoConcurrentSessionsPlusAmbientDoNotCrossContaminate(t *testing.T) {
	...
	sesA, err := mgr.Start("run-A", "svc-front", "http://"+frontProxy, "", 0)
	if err != nil {
		t.Fatal(err)
	}
	sesB, err := mgr.Start("run-B", "svc-front", "http://"+frontProxy, "", 0)
	if err != nil {
		t.Fatal(err)
	}
...
func TestPropagationGapDegradesSessionAndNamesService(t *testing.T) {
	...
	mgr := NewSessionManager(p, rec, []string{"svc-front"})
	defer mgr.Close()
	ses, err := mgr.Start("run-X", "svc-front", "http://"+frontProxy, "", 0)
	if err != nil {
		t.Fatal(err)
	}
...
func TestUnattributedMidChainTrafficMarksSuspect(t *testing.T) {
	...
	defer mgr.Close()
	ses, err := mgr.Start("run-Y", "svc-front", "http://"+frontProxy, "", 0)
	if err != nil {
		t.Fatal(err)
	}
...
func TestControlPlaneAnnotationDoesNotDegradeSession(t *testing.T) {
	...
	mgr := NewSessionManager(p, rec, []string{"svc-front"})
	defer mgr.Close()
	ses, err := mgr.Start("run-ctl", "svc-front", "http://"+frontProxy, "", 0)
	if err != nil {
		t.Fatal(err)
	}
...
func TestEndStopsEdgeAndFinalizesSession(t *testing.T) {
	...
	mgr := NewSessionManager(p, rec, []string{"svc-front"})
	defer mgr.Close()
	ses, err := mgr.Start("run-Z", "svc-front", "http://"+frontProxy, "", 0)
	if err != nil {
		t.Fatal(err)
	}
	mustGet(t, "http://"+ses.EdgeAddr+"/flow")
...
func TestSlowSubscriberDropDegradesSessionVerdict(t *testing.T) {
	...
	defer mgr.Close()
	ses, err := mgr.Start("run-drop", "svc", "http://127.0.0.1:1", "", 0) // never dialed
	if err != nil {
		t.Fatal(err)
	}
